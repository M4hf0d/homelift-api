...
class CommentsCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        comment = ProductComment.objects.filter(
            productList=productlist, comment_user=comment_user
        )
        if comment.exists():
...
class RatingCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        rating = ProductRating.objects.filter(
            productList=productlist, rating_user=rating_user
        )
        if rating.exists():
...
class SubCategoryCreateAPIView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        subcategory = SubCategory.objects.filter(
            categoryList=categorylist, name=self.request.data["name"]
        )
